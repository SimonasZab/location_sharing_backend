...
namespace location_sharing_backend.Backends {
	public class AuthBackend {
        ...
        public static DateTime GenerateJwtTokens(User user, ISecrets secrets, bool persist, HttpResponse httpResponse) {
            ...
            httpResponse.Cookies.Append(secrets.AccessTokenCookieName, accessToken, atOptions);
            ...
            httpResponse.Cookies.Append(secrets.RefreshTokenCookieName, refreshToken, rtOptions);
        ...
        public static async Task<DateTime?> RefreshAccessToken(string refreshToken, ISecrets secrets, UserService userService, HttpResponse httpResponse) {
            var jwtTokenHandler = new JwtSecurityTokenHandler();

            var tokenValidationParameters = new TokenValidationParameters {
                ValidateIssuerSigningKey = true,
                IssuerSigningKey = new SymmetricSecurityKey(secrets.JWTSecret),
            ...
            var signingCredentials = new SigningCredentials(new SymmetricSecurityKey(secrets.JWTSecret), SecurityAlgorithms.HmacSha256Signature);
            DateTime AccessTokenExpirationDate = DateTime.UtcNow.AddMinutes(15);
